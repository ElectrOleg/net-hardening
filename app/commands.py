...
def seed_command():
    ...
    for v_data in vendors:
        if not db.session.get(Vendor, v_data["code"]):
            v = Vendor(**v_data)
            db.session.add(v)

    db.session.commit()
    click.echo("Vendors seeded.")

    # 2. Policies
    policy = Policy.query.filter_by(name="Standard Hardening").first()
    if not policy:
        policy = Policy(
            name="Standard Hardening", description="Baseline security checks", severity="high"
        )
    ...
    rules_data = [
        ...
        },
        {
            ...
            "logic_type": "simple_match",
            "logic_payload": {
                "pattern": "security passwords min-length 8",
...
@click.option(
    "--password", default=None, help="Admin password (will generate a random one if omitted)"
)
@with_appcontext
