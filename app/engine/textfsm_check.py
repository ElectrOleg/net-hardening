...
class TextFSMChecker(RuleChecker):
    ...
    def get_payload_schema(cls) -> dict:
        ...
        return {
            ...
            "properties": {
                ...
                "template_name": {
                    "type": "string",
                    "description": "NTC-templates name (e.g., 'cisco_ios_show_interfaces')",
                },
                "checks": {
                    "type": "array",
                    "items": {
                ...
                "row_filter": {
                    "type": "object",
                    "properties": {"field": {"type": "string"}, "pattern": {"type": "string"}},
                },
    ...
    def check(self, config_text: str, payload: dict) -> CheckResult:
        ...
        try:
            ...
            return CheckResult.error(message="textfsm not installed. pip install textfsm")
        ...
        if template_name:
            ...
            try:
                ...
                parsed = parse.parse_output(
                    platform=template_name.rsplit("_", 1)[0],
                ...
                return CheckResult.error(message=f"Failed to use template '{template_name}': {e}")

        if not template_content:
            return CheckResult.error(message="No template content provided")
        ...
        try:
            ...
            return CheckResult.error(message=f"TextFSM parsing failed: {e}")
    ...
    def _validate_parsed_data(self, data: list[dict], payload: dict) -> CheckResult:
        ...
        if max_rows and len(data) > max_rows:
            return CheckResult.failure(message=f"Expected at most {max_rows} rows, got {len(data)}")
        ...
        for check in checks:
            ...
            if on_all:
                ...
                for i, row in enumerate(data):
                    ...
                    try:
                        if not op_func(actual, expected):
                            failures.append(
                                f"Row {i}: {field}='{actual}' failed {operator} '{expected}'"
                            )
                    except Exception as e:
                ...
                if data:
                    ...
                    failures.append("No data rows to check")
        ...
        return CheckResult.success(message=f"All {len(checks)} checks passed on {len(data)} rows")
