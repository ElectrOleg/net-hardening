...
class AdvancedBlockChecker(RuleChecker):
    ...
    def check(self, config_text: str, payload: dict) -> CheckResult:
        ...
        try:
            ...
            return CheckResult.error(message="ciscoconfparse2 not installed")
        ...
        try:
            ...
            return CheckResult.error(message=f"Config parse error: {e}")
        ...
        if not blocks:
            ...
            return CheckResult.success(message="No blocks to check")
        ...
        for block in blocks:
            ...
            if block_failures:
                all_failures.append({"block": block.block_text, "failures": block_failures})
            else:
                passed_blocks += 1
        ...
        if cross_block and not all_failures:
            ...
            if cross_failures:
                all_failures.append({"block": "Cross-block validation", "failures": cross_failures})
        ...
        return CheckResult.success(message=f"All {total_blocks} blocks passed")
    ...
    def _find_blocks(self, parse, config: dict) -> list[BlockContext]:
        ...
        for parent in parent_objs:
            ...
            blocks.append(
                BlockContext(
                    block_text=block_text, block_lines=block_lines, variables=variables, depth=0
                )
            )
    ...
    def _collect_cross_block_values(self, block: BlockContext, cross_block: dict, values: dict):
        """Collect values for cross-block validation."""
        block_content = "\n".join(block.block_lines)
        ...
        for check_type in ["all_same", "unique"]:
            ...
            for pattern in patterns:
                ...
                match = re.search(pattern, block_content, re.MULTILINE)
                if match and match.groups():
                    values[pattern].append({"block": block.block_text, "value": match.group(1)})
        ...
        for pattern in cross_block.get("all_same", []):
            if pattern in values and len(values[pattern]) > 1:
                ...
                if len(unique_values) > 1:
                    failures.append(f"Inconsistent values for '{pattern}': {unique_values}")
        ...
        for pattern in cross_block.get("unique", []):
            if pattern in values:
                ...
                if len(all_values) != len(set(all_values)):
                    ...
                    failures.append(f"Duplicate values for '{pattern}': {set(duplicates)}")
