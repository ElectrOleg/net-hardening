...
class VersionChecker(RuleChecker):
    ...
    def get_payload_schema(cls) -> dict:
        return {
            ...
            "properties": {
                ...
                "operator": {
                    "type": "string",
                    "enum": ["eq", "ne", "gt", "lt", "ge", "le", "in_range"],
                },
                ...
                "version_format": {
                    "type": "string",
                    "enum": ["semver", "numeric", "cisco", "auto"],
                },
            },
        }
    ...
    def check(self, config_text: str, payload: dict) -> CheckResult:
        ...
        try:
            ...
            if not match:
                return CheckResult.failure(message="Version pattern not found")

            actual_version = match.group(version_group)
        except Exception as e:
            return CheckResult.error(message=f"Version extraction failed: {e}")
        ...
        if passed:
            return CheckResult.success(
                message=f"Version {actual_version} {operator} {expected}", raw_value=actual_version
            )
        else:
            return CheckResult.failure(
