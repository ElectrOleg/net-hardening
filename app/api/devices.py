...
from app.models import Device, Policy, Vendor
...
def list_devices():
    ...
    if request.args.get("search"):
        ...
        query = query.filter(db.or_(Device.hostname.ilike(search), Device.ip_address.ilike(search)))
...
def create_device():
    ...
    if data.get("vendor_code") and not db.session.get(Vendor, data["vendor_code"]):
...
def update_device(device_id):
    ...
    if "vendor_code" in data:
        if data["vendor_code"] and not db.session.get(Vendor, data["vendor_code"]):
            return jsonify({"error": f"Vendor '{data['vendor_code']}' not found"}), 400
