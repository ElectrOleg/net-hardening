...
from app.models import Device, Result, Rule, Scan
...
def prometheus_metrics():
    ...
    # ── Last scan info ──────────────────────────────────────────────
    last_scan = Scan.query.filter_by(status="completed").order_by(Scan.finished_at.desc()).first()
    ...
    if last_scan:
        ...
        lines.append(
            _prometheus_line(
                "hcs_rules_passed_total",
                last_scan.passed_count,
                metric_type="gauge",
        ...
        lines.append(
            _prometheus_line(
                "hcs_rules_failed_total",
                last_scan.failed_count,
                metric_type="gauge",
        ...
        lines.append(
            _prometheus_line(
                "hcs_rules_error_total",
                last_scan.error_count,
                metric_type="gauge",
        ...
        device_stats = (
            db.session.query(Result.device_id, Result.status, func.count(Result.id))
            .filter(Result.scan_id == scan_id)
            .group_by(Result.device_id, Result.status)
            .all()
        )
        ...
        if devices:
            device_objs = Device.query.filter(
                (Device.hostname.in_(devices.keys())) | (Device.ip_address.in_(devices.keys()))
            ).all()
        ...
        lines.append("")
        lines.append("# HELP hcs_compliance_score Per-device compliance score (0-100)")
        lines.append("# TYPE hcs_compliance_score gauge")
        ...
        policy_stats = (
            db.session.query(Rule.policy_id, Result.status, func.count(Result.id))
            .join(Rule, Result.rule_id == Rule.id)
            .filter(Result.scan_id == scan_id)
            .group_by(Rule.policy_id, Result.status)
            .all()
        )
