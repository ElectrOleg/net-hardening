...
def get_scan_playbook(scan_id):
    ...
    playbook, tasks = remediation_service.generate_playbook_for_scan(str(scan_id), device_id)
    ...
    if format_type == "yaml":
        return Response(
            playbook,
            mimetype="text/yaml",
            headers={"Content-Disposition": f"attachment; filename=remediation_{scan_id}.yml"},
        )
    else:
        return jsonify(
            {
...
def generate_rule_playbook(rule_id):
    ...
    playbook = remediation_service.generate_playbook_for_rule(str(rule_id), device_ids)

    if not playbook:
        return jsonify(
            {"error": "No remediation available", "message": "Rule has no remediation commands"}
        ), 404
    ...
    if format_type == "yaml":
        return Response(
            playbook,
            mimetype="text/yaml",
            headers={"Content-Disposition": f"attachment; filename=rule_{rule_id}_remediation.yml"},
        )
    else:
...
def get_device_playbook(device_id):
    ...
    playbook, tasks = remediation_service.generate_playbook_for_scan(str(scan.id), device_id)
    ...
    return Response(
        playbook,
        mimetype="text/yaml",
        headers={"Content-Disposition": f"attachment; filename={device_id}_remediation.yml"},
    )
