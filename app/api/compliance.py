...
from app.models import Device, Policy, Result, Rule, Scan
...
def get_device_compliance(device_id):
    ...
    # Get result stats
    stats = (
        db.session.query(Result.status, func.count(Result.id))
        .filter(Result.scan_id == scan_id, Result.device_uuid == device.id)
    ...
    failures = (
        db.session.query(Result, Rule)
        .join(Rule, Result.rule_id == Rule.id)
        .filter(Result.scan_id == scan_id, Result.device_uuid == device.id, Result.status == "FAIL")
        .all()
    )
...
def get_devices_compliance():
    ...
    for key, value in request.args.items():
        if key.startswith("extra."):
            ...
            device_query = device_query.filter(Device.extra_data[json_key].astext == value)
    ...
    stats = (
        db.session.query(Result.device_uuid, Result.status, func.count(Result.id))
        .filter(Result.scan_id == scan_id, Result.device_uuid.in_(device_ids))
        .group_by(Result.device_uuid, Result.status)
        .all()
    )
...
def get_rule_compliance(rule_id):
    ...
    stats = (
        db.session.query(Result.status, func.count(Result.id))
        .filter(Result.scan_id == scan_id, Result.rule_id == rule.id)
        .group_by(Result.status)
        .all()
    )
    ...
    failed_devices = (
        db.session.query(Result.device_id, Result.device_uuid, Result.message)
        .filter(Result.scan_id == scan_id, Result.rule_id == rule.id, Result.status == "FAIL")
        .all()
    )
...
def get_policy_compliance(policy_id):
    ...
    rule_stats = (
        db.session.query(Rule.id, Rule.title, Rule.severity, Result.status, func.count(Result.id))
        .join(Result, Result.rule_id == Rule.id)
        .filter(Rule.policy_id == policy_id, Result.scan_id == scan_id)
        .group_by(Rule.id, Rule.title, Rule.severity, Result.status)
        .all()
    )
...
def get_compliance_summary():
    ...
    # By vendor
    vendor_stats = (
        db.session.query(Device.vendor_code, Result.status, func.count(Result.id))
        .join(Device, Result.device_uuid == Device.id)
        .filter(Result.scan_id == scan_id, Result.device_uuid.isnot(None))
        .group_by(Device.vendor_code, Result.status)
        .all()
    )
    ...
    vendor_scores = {
        v: _compute_score(s["PASS"], s["FAIL"], s["ERROR"]) for v, s in vendors.items()
    }

    # By location
    location_stats = (
        db.session.query(Device.location, Result.status, func.count(Result.id))
        .join(Device, Result.device_uuid == Device.id)
        .filter(Result.scan_id == scan_id, Result.device_uuid.isnot(None))
        .group_by(Device.location, Result.status)
        .all()
    )
    ...
    location_scores = {
        l: _compute_score(s["PASS"], s["FAIL"], s["ERROR"]) for l, s in locations.items()
    }

    # By severity
    severity_stats = (
        db.session.query(Rule.severity, Result.status, func.count(Result.id))
        .join(Rule, Result.rule_id == Rule.id)
        .filter(Result.scan_id == scan_id)
        .group_by(Rule.severity, Result.status)
        .all()
    )
    ...
    severity_scores = {
        s: _compute_score(d["PASS"], d["FAIL"], d["ERROR"]) for s, d in severities.items()
    }
