...
class ConfigSnapshot(db.Model):
    ...
    device_id = db.Column(
        UUID(as_uuid=True),
        db.ForeignKey("hcs_devices.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    source_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("hcs_data_sources.id", ondelete="SET NULL"), nullable=True
    )
    scan_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("hcs_scans.id", ondelete="SET NULL"), nullable=True
    )
    ...
    prev_snapshot_id = db.Column(
        UUID(as_uuid=True),
        db.ForeignKey("hcs_config_snapshots.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    def get_config_text(self) -> str | None:
        ...
        while snap and snap.prev_snapshot_id and snap.prev_snapshot_id not in visited:
            ...
            snap = db.session.get(ConfigSnapshot, snap.prev_snapshot_id)
