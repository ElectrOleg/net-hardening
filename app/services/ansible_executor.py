...
class AWXExecutor:
    ...
    def launch_job_template(
        self, template_id: int, extra_vars: Optional[dict] = None, limit: Optional[str] = None
    ) -> ExecutionResult:
        ...
        try:
            response = requests.post(
                url, headers=self.headers, json=payload, verify=self.verify_ssl, timeout=30
            )
            ...
            if response.status_code in (200, 201):
                ...
                return ExecutionResult(
                    success=False, error=f"AWX error: {response.status_code} - {response.text}"
                )
    ...
    def create_and_launch_adhoc(
        self, playbook_content: str, inventory_id: Optional[int] = None
    ) -> ExecutionResult:
        """
        ...
        return ExecutionResult(
            success=False, error="Ad-hoc playbook execution not supported. Use job templates."
        )
...
class SSHAnsibleExecutor:
    ...
    def execute(
        ...
        try:
            ...
            return ExecutionResult(
                success=False, error="netmiko not installed. pip install netmiko"
            )
        ...
        try:
            with ConnectHandler(**device) as conn:
                ...
                return ExecutionResult(success=success, status="completed", output=output)
...
class RemoteAnsibleExecutor:
    ...
    def execute(
        ...
        if self.executor_type == ExecutorType.SSH:
            return self._executor.execute(
                playbook_content, playbook_name, extra_vars, limit, check_mode
            )
    ...
    def execute_job_template(
        ...
        if self.executor_type != ExecutorType.AWX:
            return ExecutionResult(
                success=False, error="Job template execution only available in AWX mode"
            )
    ...
    def get_job_status(self, job_id: str) -> ExecutionResult:
        ...
        return ExecutionResult(
            success=False, error="Job status tracking only available in AWX mode"
        )
