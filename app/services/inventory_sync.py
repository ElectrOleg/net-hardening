...
class InventorySyncService:
    ...
    def sync(self, source: InventorySource, trigger: str = "manual") -> SyncResult:
        ...
        for rd in remote_devices:
            ...
            if not ext_id:
                result.errors.append("Skipping device with no ID or hostname")
                continue
            ...
            if vendor_code and vendor_code not in valid_vendors:
                ...
                try:
                    ...
                    logger.info(
                        f"Auto-created vendor '{vendor_code}' from inventory source '{source.name}'"
                    )
                except Exception:
                    db.session.rollback()
            ...
            try:
                device = Device.query.filter_by(external_id=ext_id, source_id=source.id).first()
    ...
    @staticmethod
    def _write_sync_log(
        source: InventorySource, result: SyncResult, start_time: datetime, trigger: str
    ):
...
class VendorDetector:
    ...
    def _get_mappings(cls, match_field: str) -> list[VendorMapping]:
        ...
        if cls._cache is None or (now - cls._cache_loaded_at) > cls._CACHE_TTL:
            try:
                cls._cache = (
                    VendorMapping.query.filter_by(is_active=True)
                    .order_by(VendorMapping.priority)
                    .all()
                )
