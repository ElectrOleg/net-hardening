...
from app.models import Policy, Result, Rule, Scan
...
class ExportService:
    ...
    def export_scan_csv(self, scan_id: str) -> str:
        ...
        # Header
        writer.writerow(
            ["Device", "Rule", "Policy", "Vendor", "Status", "Message", "Diff Data", "Checked At"]
        )
    ...
    def export_matrix_csv(self, scan_id: Optional[str] = None) -> str:
        ...
        if not scan_id:
            scan = (
                Scan.query.filter_by(status="completed").order_by(Scan.finished_at.desc()).first()
            )
            if not scan:
        ...
        results = db.session.query(Result, Rule).join(Rule).filter(Result.scan_id == scan_id).all()
    ...
    def export_failures_csv(self, scan_id: str) -> str:
        ...
        writer.writerow(["Device", "Rule", "Description", "Remediation", "Diff Data"])
    ...
    def generate_summary_report(self, scan_id: str) -> dict:
        ...
        for r in results:
            ...
            devices[r.device_id][r.status.lower()] = (
                devices[r.device_id].get(r.status.lower(), 0) + 1
            )

        # Find worst devices
        worst_devices = sorted(devices.items(), key=lambda x: x[1].get("fail", 0), reverse=True)[
            :10
        ]
        ...
        top_failing_rules = sorted(rule_failures.values(), key=lambda x: x["count"], reverse=True)[
            :10
        ]
