...
from app.models import Result, Rule
...
class RemediationService:
    ...
    VENDOR_CONNECTION_MAP = {
        ...
        "eltex_esr": {
            "network_os": "community.network.eltex_esr",
            "module": "community.network.eltex_command",
        },
        "eltex_mes": {
            "network_os": "community.network.eltex",
            "module": "community.network.cli_config",
        },
        "juniper_junos": {
            "network_os": "junipernetworks.junos.junos",
            "module": "junipernetworks.junos.junos_config",
        },
        "arista_eos": {"network_os": "arista.eos.eos", "module": "arista.eos.eos_config"},
        "huawei_vrp": {
            "network_os": "community.network.ce",
            "module": "community.network.ce_config",
        },
    ...
    def generate_playbook_for_scan(
        self, scan_id: str, device_id: Optional[str] = None
    ...
    def generate_playbook_for_rule(self, rule_id: str, device_ids: list[str]) -> str:
    ...
    def _build_network_play(
        self, device_id: str, vendor_code: str, vendor_info: dict, tasks: list[RemediationTask]
    ) -> dict:
        ...
        if vendor_code.startswith("cisco"):
            ansible_tasks.append({"name": "Save configuration", module: {"save_when": "modified"}})
    ...
    def _build_api_play(
        self, device_id: str, vendor_code: str, tasks: list[RemediationTask]
    ) -> dict:
    ...
    def _build_sample_task(self, rule: Rule, commands: list[str], vendor_info: dict) -> dict:
        ...
        if vendor_info.get("type") == "api":
            ...
            module = vendor_info.get("module", "cli_config")
            return {"name": f"Apply: {rule.title}", module: {"lines": commands}}
