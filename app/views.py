...
from app.models import DataSource, Policy, Result, Rule, Scan, Vendor
...
def dashboard():
    ...
    if active_device_ids:
        ...
        top_failing_rules = (
            db.session.query(
                Rule.id, Rule.title, Rule.severity, func.count(Result.id).label("fail_count")
            )
    ...
    stats = {"passed": passed, "failed": failed, "errors": errors, "devices": total_active_devices}
    ...
    if start_date:
        config_changes_query = config_changes_query.filter(
            ConfigSnapshot.collected_at >= start_date
        )
    config_changes_count = config_changes_query.count()

    unique_devices_sub = (
        db.session.query(Result.device_uuid)
        .join(Scan, Result.scan_id == Scan.id)
        .filter(Scan.status == "completed")
    )
    if start_date:
    ...
    if active_device_ids:
        recent_failures = (
            Result.query.join(Device, Result.device_uuid == Device.id)
            .filter(Result.status == "FAIL")
...
def scans_list():
    ...
    policies = (
        db.session.query(Policy, func.count(Rule.id).label("rule_count"))
        .outerjoin(Rule, (Rule.policy_id == Policy.id) & (Rule.is_active == True))
...
def scan_detail(scan_id):
    ...
    devices = dict(sorted(devices.items(), key=lambda x: (-x[1]["failed"], -x[1]["errors"], x[0])))
...
def device_history(device_id):
    ...
    scan_ids_with_device = (
        db.session.query(Result.scan_id).filter(Result.device_id == device_id).distinct().subquery()
    )
    scans = (
        Scan.query.filter(Scan.id.in_(scan_ids_with_device))
        .filter(Scan.status == "completed")
    ...
    for scan in scans:
        results = Result.query.filter_by(scan_id=scan.id, device_id=device_id).all()
...
def devices_list():
    ...
    devices = Device.query.filter_by(is_active=True).order_by(Device.hostname).all()
...
def rule_builder():
    ...
    clone_rule = None
    if clone_id:
        clone_rule = db.session.get(Rule, clone_id)
...
def compliance_matrix():
    ...
    scans = (
        Scan.query.filter_by(status="completed").order_by(Scan.finished_at.desc()).limit(20).all()
    )
...
def settings_devices():
    ...
    policies = Policy.query.filter_by(is_active=True).all()
    return render_template(
        "settings/devices.html", devices=devices, vendors=vendors, groups=groups, policies=policies
    )
