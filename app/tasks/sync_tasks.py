...
def auto_sync_inventory(self):
    ...
    sources = InventorySource.query.filter_by(is_active=True, sync_enabled=True).all()
    ...
    for source in sources:
        ...
        if source.last_sync_at:
            ...
            if now < next_sync_at:
                logger.debug(
                    f"Source '{source.name}' not due yet (next sync at {next_sync_at.isoformat()})"
                )
                continue

        # Run sync
        logger.info(
            f"Auto-syncing source '{source.name}' (interval={source.sync_interval_minutes}m)"
        )
        try:
...
def sync_single_source(self, source_id: str, trigger: str = "api"):
    ...
    source = db.session.get(InventorySource, source_uuid)
    if not source:
