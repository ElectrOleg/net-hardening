...
def create_app(config_class=FlaskConfig):
    ...
    def enforce_auth():
        ...
        if (
            endpoint in ("web.login_page", "web.login_submit", "web.logout", "api.api_login") or
            endpoint == "static" or
        ...
        is_admin_web = (
            endpoint == "web.admin" or
            endpoint.startswith("web.settings")
        )
        ...
        is_admin_api = (
            endpoint.startswith("api.admin_") or
            endpoint in (
            ...
            "data_sources" in endpoint or
            "inventory_sources" in endpoint or
            "vendors" in endpoint
        )
        ...
        if path.startswith("/api/") and request.method in ("POST", "PUT", "DELETE", "PATCH"):
            if endpoint not in ("api.api_logout", "api.api_me"):
                if user_role not in ("operator", "admin"):
                    return jsonify({"error": "Insufficient permissions (operator/admin required)"}), 403
