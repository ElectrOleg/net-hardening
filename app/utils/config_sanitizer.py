...
_p(r"(snmp-server\s+(?:user|host)\s+\S+\s+\S+\s+auth\s+\S+\s+)(\S+)(\s+priv\s+\S+\s+)(\S+)",
...
_p(r"(-----BEGIN\s+(?:RSA\s+)?PRIVATE\s+KEY-----).+?(-----END\s+(?:RSA\s+)?PRIVATE\s+KEY-----)",
   rf"\g<1>\n{MASK}\n\g<2>", re.DOTALL | re.IGNORECASE)
_p(r"(-----BEGIN\s+CERTIFICATE-----).+?(-----END\s+CERTIFICATE-----)",
   rf"\g<1>\n{MASK}\n\g<2>", re.DOTALL | re.IGNORECASE)
