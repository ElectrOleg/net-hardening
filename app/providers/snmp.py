...
class SNMPProvider(ConfigSourceProvider):
    ...
    def fetch_config(self, device_id: str, context: dict = None) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(success=False, error="pysnmp not installed. pip install pysnmp")

        # Build auth data
        if self.version == "3":
            auth_proto = (
                usmHMACSHAAuthProtocol if self.auth_protocol == "SHA" else usmHMACMD5AuthProtocol
            )
        ...
        try:
            errorIndication, errorStatus, errorIndex, varBinds = next(
                getCmd(SnmpEngine(), auth_data, transport, ContextData(), *oid_objects)
            )
            ...
            if errorStatus:
                return FetchResult(success=False, error=f"SNMP error: {errorStatus.prettyPrint()}")
        ...
        return FetchResult(success=True, config=json.dumps(results, indent=2), format="json")
