...
class CheckPointProvider(ConfigSourceProvider):
    ...
    def _login(self):
        ...
        resp = self._session.post(
            f"{self.base_url}/login", json=login_payload, timeout=self.timeout
        )
    ...
    def _logout(self):
        ...
        if self._session and self._sid:
            try:
                self._session.post(f"{self.base_url}/logout", json={}, timeout=10)
            except Exception:
                pass
            self._sid = None
    ...
    def _api_call(self, command: str, params: dict = None) -> dict:
        ...
        resp = self._session.post(
            f"{self.base_url}/{command}", json=params or {}, timeout=self.timeout
        )
        resp.raise_for_status()
        return resp.json()
    ...
    def _api_call_paged(self, command: str, params: dict = None) -> list:
        ...
        while True:
            ...
            objects = data.get("rulebase") or data.get("objects") or data.get("results") or []
...
class UserGateProvider(ConfigSourceProvider):
    ...
    def fetch_config(self, device_id: str = "", context: dict = None) -> FetchResult:
        try:
            ...
            return FetchResult(
                success=True, config=config, format="json", metadata={"source": "usergate"}
            )
        except Exception as e:
            return FetchResult(success=False, error=str(e))
...
class PaloAltoProvider(ConfigSourceProvider):
    ...
    def _api_request(self, params: dict) -> requests.Response:
        ...
        resp = self.session.get(f"{self.base_url}/api/", params=params, timeout=self.timeout)
        resp.raise_for_status()
        return resp
    ...
    def _fetch_json(self) -> FetchResult:
        ...
        return FetchResult(
            success=True, config=config, format="json", metadata={"source": "paloalto"}
        )
    ...
    def list_devices(self) -> list[str]:
        ...
        try:
            resp = self._api_request(
                {"type": "op", "cmd": "<show><devices><connected></connected></devices></show>"}
            )
