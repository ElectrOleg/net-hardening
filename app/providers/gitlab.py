...
class GitLabProvider(ConfigSourceProvider):
    ...
    def fetch_config(self, device_id: str, context: dict = None) -> FetchResult:
        ...
        try:
            ...
            return FetchResult(
                success=False, error=f"path_template references unknown variable: {e}"
            )
        ...
        try:
            logger.debug(
                f"Fetching config: project={self.project_id} path={file_path} ref={self.branch}"
            )
            ...
            return FetchResult(
                success=False, config=None, error=f"File not found: {file_path} ({e})"
            )
    ...
    def list_devices(self) -> list[str]:
        ...
        try:
            ...
            tree = self.project.repository_tree(
                path=self.base_path or "", ref=self.branch, recursive=True, all=True
            )
