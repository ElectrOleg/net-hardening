...
class LocalFileProvider(ConfigSourceProvider):
    ...
    def fetch_config(self, device_id: str, context: dict = None) -> FetchResult:
        ...
        if not file_path or not file_path.exists():
            return FetchResult(
                success=False, error=f"Config file not found for device: {device_id}"
            )
        ...
        try:
            ...
            return FetchResult(success=True, config=content, format=format_type)
...
class SingleFileProvider(ConfigSourceProvider):
    ...
    def fetch_config(self, device_id: str, context: dict = None) -> FetchResult:
        ...
        if not self.file_path.exists():
            return FetchResult(success=False, error=f"File not found: {self.file_path}")
