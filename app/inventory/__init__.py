...
class APIInventoryProvider(DeviceInventoryProvider):
    ...
    def test_connection(self) -> tuple[bool, str]:
        try:
            response = self._session.get(f"{self.base_url}{self.endpoint}", timeout=self.timeout)
    ...
    def list_devices(self, filters: Optional[dict] = None) -> list[InventoryDevice]:
        try:
            ...
            response = self._session.get(
                f"{self.base_url}{self.endpoint}", params=params, timeout=self.timeout
            )
            response.raise_for_status()
