...
This migration creates ALL base tables for HCS.
Uses IF NOT EXISTS to be safe on partially-initialized databases.
"""
...
def upgrade():
    ...
    # ── hcs_vendors ──────────────────────────────────────────────
    if not _table_exists(conn, "hcs_vendors"):
        op.create_table(
            "hcs_vendors",
    ...
    # ── hcs_data_sources ─────────────────────────────────────────
    if not _table_exists(conn, "hcs_data_sources"):
        op.create_table(
            "hcs_data_sources",
            sa.Column(
                "id",
    ...
    # ── hcs_policies ─────────────────────────────────────────────
    if not _table_exists(conn, "hcs_policies"):
        op.create_table(
            "hcs_policies",
            sa.Column(
                "id",
    ...
    # ── hcs_rules ────────────────────────────────────────────────
    if not _table_exists(conn, "hcs_rules"):
        op.create_table(
            "hcs_rules",
            sa.Column(
                "id",
            ...
            sa.Column(
                "policy_id", UUID(as_uuid=True), sa.ForeignKey("hcs_policies.id"), nullable=False
            ),
            sa.Column(
            ...
            sa.Column(
                "data_source_id",
                UUID(as_uuid=True),
                sa.ForeignKey("hcs_data_sources.id"),
                nullable=True,
            ),
    ...
    # ── hcs_scans ────────────────────────────────────────────────
    if not _table_exists(conn, "hcs_scans"):
        op.create_table(
            "hcs_scans",
            sa.Column(
                "id",
    ...
    if not _table_exists(conn, "hcs_inventory_sources"):
        op.create_table(
            "hcs_inventory_sources",
            sa.Column(
                "id",
    ...
    # ── hcs_device_groups ────────────────────────────────────────
    if not _table_exists(conn, "hcs_device_groups"):
        op.create_table(
            "hcs_device_groups",
            sa.Column(
                "id",
            ...
            sa.Column(
                "parent_id",
                UUID(as_uuid=True),
                sa.ForeignKey("hcs_device_groups.id"),
                nullable=True,
            ),
            sa.Column("is_active", sa.Boolean, server_default="true"),
            sa.Column("created_at", sa.DateTime, server_default=sa.func.now()),
    ...
    # ── hcs_devices ──────────────────────────────────────────────
    if not _table_exists(conn, "hcs_devices"):
        op.create_table(
            "hcs_devices",
            sa.Column(
                "id",
            ...
            sa.Column(
                "vendor_code", sa.String(50), sa.ForeignKey("hcs_vendors.code"), nullable=True
            ),
            sa.Column(
                "group_id", UUID(as_uuid=True), sa.ForeignKey("hcs_device_groups.id"), nullable=True
            ),
            ...
            sa.Column(
                "source_id",
                UUID(as_uuid=True),
                sa.ForeignKey("hcs_inventory_sources.id"),
                nullable=True,
            ),
            sa.Column("last_sync_at", sa.DateTime),
    ...
    # ── hcs_results ──────────────────────────────────────────────
    if not _table_exists(conn, "hcs_results"):
        op.create_table(
            "hcs_results",
            sa.Column(
                "id",
            ...
            sa.Column("scan_id", UUID(as_uuid=True), sa.ForeignKey("hcs_scans.id"), nullable=False),
            sa.Column("device_id", sa.String(100), nullable=False),
            sa.Column(
                "device_uuid", UUID(as_uuid=True), sa.ForeignKey("hcs_devices.id"), nullable=True
            ),
            sa.Column("rule_id", UUID(as_uuid=True), sa.ForeignKey("hcs_rules.id"), nullable=False),
    ...
    if not _index_exists(conn, "ix_hcs_users_username"):
        op.create_index("ix_hcs_users_username", "hcs_users", ["username"])

    # ── M2M: hcs_device_policies ─────────────────────────────────
    if not _table_exists(conn, "hcs_device_policies"):
        op.create_table(
            "hcs_device_policies",
            sa.Column(
                "device_id", UUID(as_uuid=True), sa.ForeignKey("hcs_devices.id"), primary_key=True
            ),
            sa.Column(
                "policy_id", UUID(as_uuid=True), sa.ForeignKey("hcs_policies.id"), primary_key=True
            ),
            sa.Column("assigned_at", sa.DateTime, server_default=sa.func.now()),
        )

    # ── M2M: hcs_group_policies ──────────────────────────────────
    if not _table_exists(conn, "hcs_group_policies"):
        op.create_table(
            "hcs_group_policies",
            sa.Column(
                "group_id",
