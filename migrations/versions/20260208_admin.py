...
def upgrade():
    ...
    if conn.dialect.name == "sqlite":
        result = conn.execute(
            sa.text("SELECT 1 FROM sqlite_master WHERE type='table' AND name='hcs_system_settings'")
        )
    ...
    if conn.dialect.name == "sqlite":
        result = conn.execute(
            sa.text("SELECT 1 FROM sqlite_master WHERE type='table' AND name='hcs_scan_schedules'")
        )
        exists = result.first() is not None
    else:
        result = conn.execute(
    ...
    if not exists:
        op.create_table(
            ...
            sa.Column("description", sa.Text),
            sa.Column(
                "cron_expression", sa.String(100), nullable=False, server_default="0 2 * * *"
            ),
