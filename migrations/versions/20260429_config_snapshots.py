...
def upgrade():
    ...
    if not _table_exists(conn, 'hcs_config_snapshots'):
        op.create_table(
            ...
            sa.Column('prev_snapshot_id', UUID(as_uuid=True),
    ...
    if not _index_exists(conn, 'ix_config_snap_device_time'):
        op.create_index('ix_config_snap_device_time', 'hcs_config_snapshots',
                        ['device_id', 'collected_at'])
    if not _index_exists(conn, 'ix_config_snap_hash'):
        op.create_index('ix_config_snap_hash', 'hcs_config_snapshots',
                        ['config_hash'])
    if not _index_exists(conn, 'ix_config_snap_scan'):
        op.create_index('ix_config_snap_scan', 'hcs_config_snapshots',
                        ['scan_id'])
