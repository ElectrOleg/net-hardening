...
from app.models import Scan, Device, ConfigSnapshot, Vendor
...
def test_at_rest_encryption(app):
    ...
    with app.app_context():
        ...
        secret_config = "hostname test-router\ninterface GigabitEthernet1\n ip address 10.0.0.1 255.255.255.0"
        snap = ConfigSnapshot(
            id=uuid.uuid4(),
            device_id=d.id,
        ...
        snap_loaded = ConfigSnapshot.query.get(snap.id)
        ...
        db.session.execute(
            db.text("INSERT INTO hcs_config_snapshots (id, device_id, config_text, config_hash, config_size, vendor_code, is_changed) "
        ...
        legacy_loaded = ConfigSnapshot.query.get(legacy_id)
...
def test_stuck_scans_valve(app):
    ...
    with app.app_context():
        ...
        # Assertions
        s1 = Scan.query.get(scan_stuck_running.id)
        ...
        s2 = Scan.query.get(scan_stuck_pending.id)
        ...
        s3 = Scan.query.get(scan_active_running.id)
        assert s3.status == "running"
        
        s4 = Scan.query.get(scan_active_pending.id)
        assert s4.status == "pending"
        
        s5 = Scan.query.get(scan_completed.id)
