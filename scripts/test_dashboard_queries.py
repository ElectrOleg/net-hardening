...
from app.models import Scan, Device, Result, Rule, Vendor, ConfigSnapshot, Policy
...
def seed_test_data():
    ...
    r_ssh = Rule(id=uuid.uuid4(), policy_id=pol.id, title="Enable SSH version 2", severity="critical", vendor_code="cisco", is_active=True, logic_type="simple_match", logic_payload={})
    r_banner = Rule(id=uuid.uuid4(), policy_id=pol.id, title="Login Banner Set", severity="medium", vendor_code="cisco", is_active=True, logic_type="simple_match", logic_payload={})
    r_snmp = Rule(id=uuid.uuid4(), policy_id=pol.id, title="SNMP Public Community Disabled", severity="high", vendor_code="juniper", is_active=True, logic_type="simple_match", logic_payload={})
    r_ntp = Rule(id=uuid.uuid4(), policy_id=pol.id, title="NTP Configured", severity="low", vendor_code="eltex", is_active=True, logic_type="simple_match", logic_payload={})
    ...
    s1 = Scan(id=uuid.uuid4(), started_by="cron", status="completed", 
              started_at=now - timedelta(days=5), finished_at=now - timedelta(days=5),
              passed_count=1, failed_count=1, error_count=0, total_devices=2)
    # Scan 2: 2 days ago, completed (D1 scanned)
    s2 = Scan(id=uuid.uuid4(), started_by="admin", status="completed", 
              started_at=now - timedelta(days=2), finished_at=now - timedelta(days=2),
              passed_count=2, failed_count=0, error_count=0, total_devices=1)
    # Scan 3: 1 day ago, completed (D2, D3 scanned)
    s3 = Scan(id=uuid.uuid4(), started_by="admin", status="completed", 
    ...
    res1 = Result(id=uuid.uuid4(), scan_id=s1.id, device_uuid=d1.id, device_id=d1.hostname, rule_id=r_ssh.id, status="PASS", checked_at=s1.finished_at)
    res2 = Result(id=uuid.uuid4(), scan_id=s1.id, device_uuid=d2.id, device_id=d2.hostname, rule_id=r_snmp.id, status="FAIL", checked_at=s1.finished_at)
    
    # Scan 2: D1 passed both r_ssh & r_banner (latest results for D1)
    res3 = Result(id=uuid.uuid4(), scan_id=s2.id, device_uuid=d1.id, device_id=d1.hostname, rule_id=r_ssh.id, status="PASS", checked_at=s2.finished_at)
    res4 = Result(id=uuid.uuid4(), scan_id=s2.id, device_uuid=d1.id, device_id=d1.hostname, rule_id=r_banner.id, status="PASS", checked_at=s2.finished_at)
    
    # Scan 3: D2 passed r_snmp (latest result for D2); D3 failed r_ntp (latest result for D3)
    res5 = Result(id=uuid.uuid4(), scan_id=s3.id, device_uuid=d2.id, device_id=d2.hostname, rule_id=r_snmp.id, status="PASS", checked_at=s3.finished_at)
    res6 = Result(id=uuid.uuid4(), scan_id=s3.id, device_uuid=d3.id, device_id=d3.hostname, rule_id=r_ntp.id, status="FAIL", checked_at=s3.finished_at)
    
    db.session.add_all([res1, res2, res3, res4, res5, res6])
    
    # 6. Config Snapshots
    # D1 snapshot in Scan 1
    snap1 = ConfigSnapshot(id=uuid.uuid4(), device_id=d1.id, scan_id=s1.id, config_hash="h1", is_changed=True, collected_at=s1.finished_at, vendor_code="cisco")
    # D1 snapshot in Scan 2 (changed config)
    snap2 = ConfigSnapshot(id=uuid.uuid4(), device_id=d1.id, scan_id=s2.id, config_hash="h2", is_changed=True, collected_at=s2.finished_at, vendor_code="cisco")
    # D2 snapshot in Scan 3 (unchanged config)
    snap3 = ConfigSnapshot(id=uuid.uuid4(), device_id=d2.id, scan_id=s3.id, config_hash="h3", is_changed=False, collected_at=s3.finished_at, vendor_code="juniper")
...
def run_dashboard_test():
    ...
    with app.app_context():
        ...
        assert config_changes_count == 2, f"Expected 2 changed config snapshots (snap1, snap2), got {config_changes_count}"
    ...
    logger.info("🎉 All dashboard queries verified and passed successfully on SQLite mockup database!")
