...
from app import db
from app.models import Device, Scan
...
def test_auto_run_scheduled_scans(app, session):
    ...
    with (
        patch("app.utils.distributed_lock.get_lock") as mock_get_lock,
        ...
        scan = db.session.get(Scan, uuid.UUID(called_scan_id))
        ...
        updated_schedule = db.session.get(ScanSchedule, schedule.id)
        assert updated_schedule.last_run_at is not None
        assert updated_schedule.last_scan_id == scan.id
        assert updated_schedule.next_run_at > utc_now()
...
def test_get_devices_from_inventory_filtering(app, session):
    ...
    devices = service._get_devices_from_inventory(
        rules=None, devices_filter={"vendor": "cisco_ios"}
    )
    assert set(devices) == {"router1", "router2"}

    # Test case 3: Group filter (UUID object)
    devices = service._get_devices_from_inventory(
        rules=None, devices_filter={"group_id": group_a.id}
    )
    assert set(devices) == {"router1", "switch1"}

    # Test case 4: Group filter (string UUID representation)
    devices = service._get_devices_from_inventory(
        rules=None, devices_filter={"group_id": str(group_a.id)}
    )
    assert set(devices) == {"router1", "switch1"}

    # Test case 5: Combined vendor and group filters
    devices = service._get_devices_from_inventory(
        rules=None, devices_filter={"vendor": "cisco_ios", "group_id": str(group_a.id)}
    )
